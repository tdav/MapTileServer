    ...
    public class Style
    {
        ...
        public Brush ParseStyle(Layer layer, double scale, Dictionary<string, object> attributes)
        {
            ...
            if (paintData != null)
            {
                ...
                // string s = string.Join(";", paintData.Select(x => x.Key + "=" + x.Value).ToArray());
                // Console.WriteLine($"Paint: {s}");
            ...
            if (layoutData != null)
            {
                ...
                if (layoutData.ContainsKey("text-offset"))
                {
                    ...
                    paint.TextOffset = new Point(Convert.ToDouble(value[0], CultureInfo.InvariantCulture) * scale, Convert.ToDouble(value[1], CultureInfo.InvariantCulture) * scale);
                }
                ...
                {
                    paint.IconScale = Convert.ToDouble(getValue(layoutData["icon-size"], attributes), CultureInfo.InvariantCulture) * scale;
                }
                ...
                //  string s = string.Join(";", layoutData.Select(x => x.Key + "=" + x.Value).ToArray());
                //  Console.WriteLine($"Layout: {s}");
        ...
        private Color parseColor(object iColor)
        {
            ...
            if (colorString.StartsWith("hsl("))
            {
                ...
                double h = double.Parse(segments[1], CultureInfo.InvariantCulture);
                double s = double.Parse(segments[2], CultureInfo.InvariantCulture);
                double l = double.Parse(segments[3], CultureInfo.InvariantCulture);
            ...
            if (colorString.StartsWith("hsla("))
            {
                ...
                double s = double.Parse(segments[2], CultureInfo.InvariantCulture);
                double l = double.Parse(segments[3], CultureInfo.InvariantCulture);
                double a = double.Parse(segments[4], CultureInfo.InvariantCulture) * 255;
            ...
            {
                var segments = colorString.Replace('%', '\0').Split(',', '(', ')');
                double r = double.Parse(segments[1], CultureInfo.InvariantCulture);
                double g = double.Parse(segments[2], CultureInfo.InvariantCulture);
                double b = double.Parse(segments[3], CultureInfo.InvariantCulture);
                double a = double.Parse(segments[4], CultureInfo.InvariantCulture) * 255;
